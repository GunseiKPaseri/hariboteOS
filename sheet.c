...
void sheet_slide(struct SHEET *sht, int vx0, int vy0)
{
	...
	if (sht->height >= 0) { /* もしも表示中なら */
		/* 新しい下じきの情報に沿って画面を描き直す */
		sheet_refreshsub(sht->ctl, old_vx0, old_vy0, old_vx0 + sht->bxsize, old_vy0 + sht->bysize); /* 移動前の範囲 */
		sheet_refreshsub(sht->ctl, vx0, vy0, vx0 + sht->bxsize, vy0 + sht->bysize);/* 移動後の範囲  */
	}
	return;
}
